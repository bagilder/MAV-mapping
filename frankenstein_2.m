...
threshold = 30; %the value above which the sonar will ignore data %arbitrary
outlier_threshold = 1;
hold_flag_left=0;
hold_flag_right=0;
buffer_count=0;
...
while((xRecord(1) ~= xRecord(length(xRecord))||(yRecord(1) ~= yRecord(length(yRecord)))||(thRecord(1) ~= thRecord(length(thRecord)))))
...
data = [data; xPos,yPos,theta,sensorDistLeft,sensorDistRight];
...
buffer_left(6)= x_temp_left;
buffer_right(6) = x_temp_right;
...
if(buffer_count>5)
    ...
    display_right=buffer_right(6);
        ...
        display_right= 100000;
    end
    
    if((buffer_left(6)<(buffer_left(5) + outlier_threshold))||(buffer_left(5)>(buffer_left(5) - outlier_threshold)))
...
plot(-y_temp_left+86,display_left+10,'ok','markersize', 2)
plot(-y_temp_right+86,display_right+10,'ok','markersize', 2)
plot(-yPos+86,xPos+10,'.r','markersize', 8)
...
xRecord = cat(2, xPos, xRecord(1:length(xRecord)-1));
yRecord = cat(2, yPos, yRecord(1:length(yRecord)-1));
thRecord = cat(2, theta, thRecord(1:length(thRecord)-1));
end
hold off
end
fclose(s);
